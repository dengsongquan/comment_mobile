    ...
    <div class="right1">
      ...
      <div v-if="personData.isCurrentUser == true" class="rightMain">
        <div class="personsPhoto">
          <img :src="imgSrc + personInfo.employeeId" :onerror="headPortrait" @click=" showPop = true " />
          ...
          <van-list :finished="finished" finished-text="没有更多了" style="height: 530px;overflow: scroll;margin: 1rem 1rem;">
            <van-row>
      ...
      <div v-if="personData.isCurrentUser == false" class="rightMain">
        ...
        <van-list :finished="finished" finished-text="没有更多了" style="height: auto;overflow: scroll;margin: 1rem 1rem;">
          <van-row>
            <van-col span="12" v-for="(item, index) in questionList" :key="index" style="width: 100% ;">
              <div style="background: white;text-align: left;">
                ...
                <div v-if="item.typeId == 1" v-for="items in item.answers" :key="items.id">
                  <van-radio-group  style="margin:10px 0;"  v-model="requiredRadio[index]">
                ...
                <van-field :name=" requiredRadio[index] = textaresList[index] + '/' + personInfo.departmentId +'/' + personInfo.eid + '/' + item.questionnaireId " @input="blankDescArea" required maxlength="800" v-if="item.typeId == 2" v-model="textaresList[index]" placeholder="请简要说明">
        ...
        <van-button :disabled="isDisable" type="info" block @click="submit">提交</van-button>
