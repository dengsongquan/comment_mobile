...
<!--        教师层级  -->
        <van-tabs v-model="employeeTab" @click="employeeClassResult" >
          ...
          <van-tab v-if="isParticipation != 0" v-for="item in employeeClassData" :key="item.id" :title="item.text" :name="item.text">
                <div v-if=" item.text === employeeTab" class="rightMain">
                  <p class="totalEvaluate">
                    参评人数：
                    <span>{{departentResult.totalEvaluate}}</span>人
                  </p>
                  <div class="chartsBox" style="position:relative;left: 5%;">
                    <v-chartRadar
                  ...
                  <div v-show="index != 0" style="position:relative;left: 5%;"
      ...
      this.axios.get(url).then(res => {
        if (res.data.data != null){
          ...
          this.departmentName = this.departentResult.departmentName;
            ...
            this.indicatorName = [];
            this.opinionValue = [];
            this.radarData.dataInfoList.forEach(i => {
              if (i.percent != null){
                this.indicatorName.push({
                  max: 100,
                  name: i.name
                });
                this.opinionValue.push(i.percent)
              }
            });
